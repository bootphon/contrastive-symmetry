...
#TOTALS_REST <- read.csv("../TOTALS_REST.csv")
#TOTALS_ <- rbind(TOTALS[,c(1,2,7:10)], TOTALS_REST)
TOTALS_ <- read.csv("../TOTALS_REST.csv")
```

```{r process-input, include=FALSE, cache=TRUE}
normalize <- function(v, mn, mx, fallback) {
  ifelse(mx==mn, fallback, (v-mn)/(mx-mn))
}
registerDoParallel()
stats_by_i <- merge(minfeat_r, size_r)
#stats_by_i <- merge(stats_by_i, TOTALS_)
stats_by_i$ec <- with(stats_by_i, size/(2^nfeat))

tbalance_by_si <- ddply(tbalance_r, .(language, segment_type, inventory_type,
                                     spec_id),
                       summarize, stb=sum(tbalance), .parallel=T)
stats_by_i_ <- merge(stats_by_i, tbalance_by_i)
stats_by_i_$fb <- with(stats_by_i_, 1.-normalize(stb, min_sum_ssegs, max_sum_ssegs, 0))
  

tbalance_by_fi <- merge(stats_by_i, tbalance_r)

tbalance_by_i <- ddply(tbalance_by_fi, .(language, segment_type, inventory_type),
                       summarize, stb=sum(tbalance), .parallel=T)
stats_by_i_ <- merge(stats_by_i, tbalance_by_i)
stats_by_i_$fb <- with(stats_by_i_, 1.-normalize(stb, min_sum_ssegs, max_sum_ssegs, 0))
  
#stats_by_fi$fb <- with(stats_by_fi, 1.-normalize(tbalance, min_min_ssegs,
#                                                 max_max_ssegs, 0))

pcount_by_fsi <- merge(pcount_r, size_r)
pcount_by_si <- ddply(pcount_by_fsi, .(language, segment_type, inventory_type,
                                       spec_id),
                      summarize, spc=sum(pair_count), .parallel=T)
pcount_by_si_ <- merge(pcount_by_si, stats_by_i)
pcount_by_si_$mp <- with(pcount_by_si_, normalize(spc, min_sum_pairs, max_sum_pairs, 1))
pcount_by_i <- ddply(pcount_by_si_, .(language, segment_type, inventory_type),
                          summarize, mpnn=median(mp), .parallel=T)
#tb_by_i <- ddply(stats_by_fi, .(language, segment_type, inventory_type), summarize,
#                 fb=mean(fb), .parallel=T)

stats_by_i <- merge(stats_by_i_, pcount_by_i)
...
minfeat_d <- unique(minfeat_d[,1:2])
tbalance_d <- read.csv(D_TBALANCE_FILENAME)
pcount_d <- read.csv(D_PCOUNT_FILENAME)

stats_d <- merge(minfeat_d, size_d)
stats_d <- merge(stats_d, TOTALS_)
stats_d$ec <- with(stats_d, size/(2^nfeat))
tb_d <- merge(tbalance_d, stats_d)
tb_d_ <- ddply(tb_d, .(language), summarize, sst=sum(tbalance))
tb_d <- merge(stats_d, tb_d_)
tb_d$fb <- with(tb_d, 1.-normalize(sst, min_sum_ssegs, max_sum_ssegs, 0))
...
```{r ec-whole,echo=FALSE, fig.width=30, fig.height=15}
...
d_summ <- ddply(filter(d, segment_type=="Whole"),
                       .(segment_type, inventory_type), summarize,
                ec_mean=mean(ec))
p <- ggplot(filter(d, segment_type=="Whole"), aes_string(x="ec")) +
      ...
      geom_vline(data=d_summ, aes(xintercept=ec_mean, group=inventory_type),
                 lwd=2.5, alpha=1.0) 
p1 <- plot_prevalence(filter(d, segment_type=="Whole"), "ec", "inventory_type",
                     "Economy (Ec)", "Inventory Type",
                     point_size=4.0, line_width=2.0, text_size=36,
                     change_font=T, initial_plot=p, bins=31)  +
...
```{r ec-all-big,echo=FALSE, fig.width=30, fig.height=15}
d <- one_random(stats_by_i, "Random Segment")
d$segment_type <- factor(d$segment_type, levels=c("Whole", "Consonant",
                                                  "Stop", "Vowel"))
d$inventory_type <- factor(d$inventory_type, levels=c("Random", "Natural"))
...
print(p1)
```
```{r ec-roc-whole,echo=TRUE}
d <- one_random(stats_by_i, "Random Segment")
d$segment_type <- factor(d$segment_type, levels=c("Whole", "Consonant",
                                                  "Stop", "Vowel"))
d$inventory_type <- factor(d$inventory_type, levels=c("Random", "Natural"))
d <- filter(d, segment_type=="Whole")
rocs <- ddply(d, .(segment_type),
              function(dd) pred_stats(dd$ec, dd$inventory_type))
p <- ggplot(rocs, aes(x=fpr, y=tpr)) +
       geom_abline(slope=1.0, intercept=0.0, linetype="dashed", size=3.2) +
       geom_line(size=3.2) +
       theme(legend.position="bottom") + ylim(c(0,1)) + 
       theme(text=element_text(size=36), axis.text=element_text(size=36)) +
...
```{r fb-big,echo=FALSE, fig.width=30, fig.height=15}
d <- one_random(stats_by_i, "Random Segment")
d$segment_type <- factor(d$segment_type, levels=c("Whole", "Consonant",
                                                  "Stop", "Vowel"))
d$inventory_type <- factor(d$inventory_type, levels=c("Random", "Natural"))
...
p <- ggplot(d, aes_string(x="mpnn")) +
      ...
      geom_vline(data=d_summ, aes(xintercept=mp_mean, group=inventory_type),
                 lwd=0.5, alpha=1.0) 
p3 <- plot_prevalence(d, "mpnn", "inventory_type",
...
```{r mp-big,echo=FALSE, fig.width=30, fig.height=15}
d <- one_random(stats_by_i, "Random Segment")
d$segment_type <- factor(d$segment_type, levels=c("Whole", "Consonant",
                                                  "Stop", "Vowel"))
d$inventory_type <- factor(d$inventory_type, levels=c("Random", "Natural"))
d_summ <- ddply(d, .(segment_type, inventory_type), summarize,
                mpnn_mean=mean(mpnn))
p <- ggplot(d, aes_string(x="mpnn")) +
      geom_vline(data=d_summ, aes(xintercept=mpnn_mean, colour=inventory_type),
                 lwd=1.5, alpha=0.8) +
      geom_vline(data=d_summ, aes(xintercept=mpnn_mean, group=inventory_type),
                 lwd=0.5, alpha=1.0) 
p2 <- plot_prevalence(d, "mpnn", "inventory_type",
                     "Loc", "Inventory Type",
                     point_size=4.0, line_width=2.0, text_size=36,
                     change_font=T, initial_plot=p, bins=31)  +
      theme(plot.margin = unit(c(0.5,1.5,0.5,0.5), "cm")) +
...
d <- one_random(stats_by_i, "Random Segment")
ddply(d, .(segment_type),
      .fun=partial(auc_by_inventory_type, measure="mpnn"))
```
...
d$inventory_type <- factor(d$inventory_type, levels=c("Natural",
                                                      ...
                                                      "Random Matrix"))
levels(d$inventory_type)[2] <- "Control"
levels(d$inventory_type)[3] <- "Weighted"
levels(d$inventory_type)[4] <- "Uniform"

d_means <- ddply(d, .(inventory_type, segment_type), summarize,
                      ec=mean(ec), fb=mean(fb), mpnn=mean(mpnn))
d_control <- filter(d_means, inventory_type == "Uniform")
d_natural_vc <- filter(d_means, inventory_type == "Natural")
d_newrand_vc <- filter(d_means, inventory_type %in% c("Weighted", "Control"))
d_control <- melt(d_control, measure.vars=c("ec", "fb", "mpnn"),
                     variable_name = "measure")
d_natural_vc <- melt(d_natural_vc, measure.vars=c("ec", "fb", "mpnn"),
                     variable_name = "measure")
d_newrand_vc <- melt(d_newrand_vc, measure.vars=c("ec", "fb", "mpnn"),
                     variable_name = "measure")
...
levels(d$inventory_type)[3] <- "Asymmetrical"
levels(d$inventory_type)[4] <- "Uniform"
d <- melt(d, measure.vars=c("ec", "fb", "mp", "mpnn"), variable_name = "measure")
